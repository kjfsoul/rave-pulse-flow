...
// Default audio file - falls back gracefully if not found
export const useAudioPlayer = (audioSrc: string | null = null): AudioPlayerHook => {
  const defaultSrc = '/audio/festival_mix.mp3';
  const actualSrc = audioSrc || defaultSrc;
  ...
  useEffect(() => {
    ...
    audio.addEventListener('error', async () => {
      console.warn(`Audio file ${actualSrc} not found, using generated audio`);
      try {
  ...
  }, [actualSrc, state.volume, state.bpm]);
