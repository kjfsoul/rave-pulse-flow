 ...
 *
 * Usage: Call once at the app root level or in ProductionStation to initialize the audio context.
 *
 * Adapted from vFLX-10 Pro Station to use useProStationStore instead of useGlobalStore.
 */
export function useToneTransport() {
  const { masterBPM, isPlaying, masterVolume } = useProStationStore();
  const initialized = useRef(false);

  // Initialize Tone.js once
  useEffect(() => {
    if (!initialized.current) {
      // Set initial BPM and volume from store
