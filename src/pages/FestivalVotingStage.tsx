...
const FestivalVotingStage = () => {
  ...
  const [chatMessages, setChatMessages] = useState<
    Array<{ id: string; username: string; message: string; timestamp: Date }>
  >([]);
  const [newMessage, setNewMessage] = useState("");
  const [lightBurst, setLightBurst] = useState(false);
  const currentArchetype = (profile?.archetype || "MoonWaver") as
    | "Firestorm"
    | "FrostPulse"
    | "MoonWaver";
  ...
  const handleVote = async (djId: number) => {
    ...
    try {
      ...
      const result = await festivalOperations.submitVote(
        user.id,
        djData.name,
        voteWeight
      );
      ...
      if (result.success) {
        ...
        if (voteWeight === 2) {
          toast.success(
            `Double vote for ${djData.name}! Archetype match bonus!`,
            {
              icon: "⚡",
            }
          );
        ...
        if (
          ...
          result.error?.includes("frequently")
        ) {
          toast.error(`⏰ ${result.error}`, {
            description: result.nextVoteAllowed
              ? `Try again after ${new Date(
                  result.nextVoteAllowed
                ).toLocaleString()}`
              : undefined,
            duration: 5000,
          });
      ...
      for (let channel = 0; channel < 2; channel++) {
        ...
        for (let i = 0; i < channelData.length; i++) {
          ...
          if (type === "scratch") {
            ...
            const baseFreq = 110;
            const echo1 =
              Math.sin(time * baseFreq * 2 * Math.PI) * Math.exp(-time * 2);
            const echo2 =
              Math.sin((time - 0.2) * baseFreq * 2 * Math.PI) *
              Math.exp(-(time - 0.2) * 2) *
              0.5;
            const echo3 =
              Math.sin((time - 0.4) * baseFreq * 2 * Math.PI) *
              Math.exp(-(time - 0.4) * 2) *
              0.25;
            sample =
              (echo1 + (time > 0.2 ? echo2 : 0) + (time > 0.4 ? echo3 : 0)) *
              0.3;
          }
  ...
  useEffect(() => {
    ...
    const headlinerId = Object.entries(votes).find(
      ([_, v]) => v === maxVotes
    )?.[0];
  ...
  return (
    <Layout title="EDM Shuffle Mainstage">
      <div className="min-h-screen bg-bass-dark relative overflow-hidden">
        ...
        />
        <ShuffleDancers bpm={currentBPM} dancerCount={8} intensity="high" />
        <ArchetypeAuraSprite
          archetype={currentArchetype}
          bpm={currentBPM}
          intensity={90}
        ...
        <div className="absolute inset-0 opacity-30">
          ...
          <motion.div
            ...
            animate={{
              background: selectedDJData
                ? `linear-gradient(135deg, ${selectedDJData.gradient
                    .replace("from-", "")
                    .replace(" via-", ", ")
                    .replace(" to-", ", ")} 0.1)`
            ...
            <motion.div
              ...
              style={{
                background: `conic-gradient(from ${
                  i * 60
                }deg, transparent 0%, rgba(191, 90, 242, 0.3) 1%, transparent 2%)`,
              }}
          ...
          {/* Drone Formations */}
          <DroneFormations isActive={true} className="z-5" />
        </div>
        ...
        <div className="relative z-10 p-4 pt-8">
          ...
          >
            ...
            <p className="text-neon-cyan text-lg">
              Live from the Digital Ultra Arena
            </p>
          </motion.div>
          ...
          >
            <div className="text-sm text-neon-cyan">
              {profile?.archetype === "Firestorm"
                ...
                ? "🌙"
                : "🎴"}{" "}
              {profile?.username || "Raver"}
            </div>
            <div className="text-xs text-slate-400">
              {profile?.archetype || "Festival Supporter"} • Level{" "}
              {profile?.level || 1}
            </div>
          </motion.div>
            ...
            >
              <div className="text-center mb-6">
                ...
                <p className="text-slate-300 mb-4">
                  Discover up-and-coming DJs and live performers
                </p>
                ...
                <div className="flex justify-center gap-6 text-sm text-slate-400">
                  <div>
                    🔴 <span className="text-red-400">1</span> Live
                  </div>
                  <div>
                    👥 <span className="text-neon-cyan">47.2K</span> Watching
                  </div>
                  <div>
                    🎵{" "}
                        ...
                        <div className="relative z-10">
                          ...
                          <div className="text-4xl mb-2">
                            {dj.archetype === "Firestorm"
                              ? "🔥"
                              : dj.archetype === "FrostPulse"
                              ? "❄️"
                              : "🌙"}
                          </div>
                          <h3 className={`font-bold ${dj.color} mb-1`}>
                            {dj.name}
                          </h3>
                          ...
                          <div className="text-xs mb-3">
                            {dj.achievements
                              ...
                              .map((achievement, i) => (
                                <div
                                  key={i}
                                  className="text-neon-cyan bg-neon-cyan/20 px-2 py-1 rounded-full inline-block"
                                >
                                  {achievement}
                                </div>
                              ))}
                          ...
                          <div className="space-y-2">
                            <NeonButton
                              ...
                              variant={
                                selectedDJ === dj.id ? "primary" : "secondary"
                              }
                            >
                              {selectedDJ === dj.id && isPlaying
                                ? "🔊 Playing"
                                : "🎧 Preview"}
                            </NeonButton>
                            <NeonButton
                              variant={
                                userVotes.has(dj.name) ? "secondary" : "primary"
                              }
                              size="sm"
                            ...
                            {profile?.archetype === dj.archetype &&
                              !userVotes.has(dj.name) && (
          ...
          >
            <motion.button
              ...
              whileHover={{
                scale: 1.1,
                boxShadow: "0 0 30px rgba(191, 90, 242, 0.6)",
              }}
              whileTap={{ scale: 0.9 }}
              onClick={() => {
                setShowConfetti(true);
