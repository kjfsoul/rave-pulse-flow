...
import { activityOperations, profileOperations } from "@/lib/database";
...
const quizQuestions: QuizQuestion[] = [
  {
    ...
    answers: [
      {
        text: "🔥 Explodes with aggressive stomps and sharp cuts",
        archetype: "Firestorm",
        vibe: "fire",
      },
      {
        text: "❄️ Flows like liquid ice with smooth precision",
        archetype: "FrostPulse",
        vibe: "frost",
      },
      {
        text: "🌙 Waves and rolls with dreamy, hypnotic motion",
        archetype: "MoonWaver",
        vibe: "wave",
      },
    ],
  },
  {
    ...
    answers: [
      {
        text: "🔥 Blazing LED armor and fire-colored gear",
        archetype: "Firestorm",
        vibe: "fire",
      },
      {
        text: "❄️ Crystalline whites and electric blue accents",
        archetype: "FrostPulse",
        vibe: "frost",
      },
      {
        text: "🌙 Holographic pastels and cosmic patterns",
        archetype: "MoonWaver",
        vibe: "wave",
      },
    ],
  },
  {
    ...
    answers: [
      {
        text: "🔥 The crowd goes wild and chaos takes over",
        archetype: "Firestorm",
        vibe: "fire",
      },
      {
        text: "❄️ You find perfect sync with the beat's precision",
        archetype: "FrostPulse",
        vibe: "frost",
      },
      {
        text: "🌙 You lose yourself in the music's emotional journey",
        archetype: "MoonWaver",
        vibe: "wave",
      },
    ],
...
const ArchetypeQuiz = () => {
    ...
    if (currentStep < quizQuestions.length - 1) {
      ...
      if (user) {
        ...
        try {
          ...
          const archetypeSaved = await profileOperations.updateArchetype(
            user.id,
            winner
          );
          if (!archetypeSaved) {
          ...
          // Update PLUR points
          const pointsUpdated = await profileOperations.updatePLURPoints(
            user.id,
            profile.plur_points + 50
          );
          if (!pointsUpdated) {
          ...
          toast.success(
            `Archetype "${winner}" saved to your profile! +50 PLUR points awarded!`
          );
        } catch (error) {
          console.error("Error saving archetype:", error);
          console.error("Error details:", JSON.stringify(error, null, 2));
          toast.error(
            `Failed to save archetype: ${error.message || "Unknown error"}`
          );
        } finally {
          setIsSaving(false);
        }
  ...
  if (showReveal && finalArchetype) {
    return (
      <ProtectedRoute>
        <Layout title="Archetype Reveal">
          <div className="min-h-screen bg-bass-dark relative pb-20">
            <ArchetypeRevealCeremony
              archetype={
                finalArchetype as "Firestorm" | "FrostPulse" | "MoonWaver"
              }
