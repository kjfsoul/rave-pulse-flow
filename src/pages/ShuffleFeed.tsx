...
import { Gamepad2, Heart, Play, Share2, Sparkles, Users } from "lucide-react";
...
const ShuffleFeed = () => {
  ...
  const handleCardInteraction = async (challenge: Challenge) => {
    ...
    if (activeChallenges.includes(challenge.title)) {
      ...
      try {
        const activeChallenge = await challengeOperations.getUserChallenges(
          user.id
        );
        const challengeToComplete = activeChallenge.find(
          (c) => c.challenge_type === challenge.title && c.status === "pending"
        );

        if (challengeToComplete) {
          await challengeOperations.completeChallenge(
          ...
          setActiveChallenges((prev) =>
            prev.filter((c) => c !== challenge.title)
          );

          toast.success(
            `Challenge completed! +${challenge.plurPower} PLUR points!`,
            {
              icon: "🎉",
            }
          );
    ...
    if (Math.random() < 0.2 && completedChallenges.includes(challenge.title)) {
      ...
      const archetypes: ("Firestorm" | "FrostPulse" | "MoonWaver")[] = [
        "Firestorm",
        "FrostPulse",
        "MoonWaver",
      ];
    ...
    const loadUserCrew = async () => {
      ...
      try {
        ...
        const formattedCrew: PLURcrewMember[] = crewData.map((member) => ({
          ...
          avatar:
            member.crew_member?.archetype === "Firestorm"
              ? "🔥"
              : member.crew_member?.archetype === "FrostPulse"
              ? "❄️"
              : "🌙",
        }));
  ...
  useEffect(() => {
    const loadChallengeHistory = async () => {
      ...
      try {
        const userChallenges = await challengeOperations.getUserChallenges(
          user.id
        );
          ...
          >
            <div className="flex gap-1">
              {plurCrew.slice(0, 3).map((member) => (
                <div key={member.id} className="text-lg">
                  {member.avatar}
                </div>
              ))}
            </div>
            ...
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-slate-400">
                  Master the moves, evolve your crew
                </p>
              </div>
          ...
          <div className="space-y-4">
            {challenges.map((challenge, index) => {
              ...
              return (
                ...
                >
                  <Card
                    className={`${colors.bg} border-2 ${colors.border} ${colors.glow} shadow-xl transition-all duration-300 group overflow-hidden relative`}
                  >
                    {/* Archetype Glow Ring */}
                    <motion.div
                      ...
                      transition={{
                        duration: 60000 / challenge.bpm / 1000,
