...
const RSSWebSocketManager: React.FC<RSSWebSocketManagerProps> = ({
          ...
          try {
            ...
            if (status === "SUBSCRIBED") {
              ...
              const errorReason = err
                ...
                : "Connection failed";
              console.error(
                `Subscription failed: ${status}. Reason: ${errorReason}`
              );
  ...
  const handleReconnection = useCallback(() => {
    if (reconnectAttempts.current >= maxReconnectAttempts) {
      ...
      const finalError =
        "Could not connect to the live feed. The server may be temporarily down.";
    ...
    const delay = Math.min(
      1000 * Math.pow(2, reconnectAttempts.current) + Math.random() * 1000,
      45000
    );

    console.log(
      `Attempting to reconnect in ${Math.round(delay / 1000)}s (attempt ${
  ...
  useEffect(() => {
    ...
    const handleVisibilityChange = () => {
      if (!document.hidden && subscriptionRef.current?.state !== "joined") {
        console.log(
          "Tab became visible, checking and refreshing connection..."
        );
        checkConnection();
      }
    };
